    ...
    .map((feature) => {
      const coordinates =
        feature.geometry.type === "Point"
          ? feature.geometry.coordinates.map((value) =>
...
test("streaming PMTiles generation preserves MVT contents and publishes only archive + manifest", async () => {
  ...
  try {
      ...
      features: [
        {
          ...
          properties: { DEPTH: 21, SOURCE_CELL: "test" },
          geometry: { type: "Point" as const, coordinates: [-80.15, 25.7] },
        },
        ...Array.from({ length: 40 }, (_, i) => ({
          type: "Feature" as const,
          properties: { DEPTH: i, SOURCE_CELL: "adjacent" },
          geometry: {
            type: "Point" as const,
            coordinates: [-80.18 + i * 0.003, 25.68 + i * 0.002],
          },
        })),
      ],
    };
    ...
    const builder = fileURLToPath(
      new URL("../scripts/build-soundg-tiles.ts", import.meta.url),
    );
    ...
    const expected = Buffer.from(
      vtpbf.fromGeojsonVt({ soundings: index.getTile(14, 4544, 6981)! }),
    );
    await assertEquivalentMvt(
      await storage.getTile("soundg", "v1", 14, 4544, 6981),
      expected,
      directory,
      14,
      4544,
    ...
    for (let z = 8; z <= 16; z++) {
      ...
      for (let tx = x(-80.18) - 1; tx <= x(-80.063) + 1; tx++) {
        for (let ty = y(25.758) - 1; ty <= y(25.68) + 1; ty++) {
          ...
          const bytes = tile?.features.length
            ? Buffer.from(vtpbf.fromGeojsonVt({ soundings: tile }))
            : undefined;
          assert.equal(
            Boolean(await storage.getTile("soundg", "v1", z, tx, ty)),
            Boolean(bytes),
            `${z}/${tx}/${ty}`,
          );
        }
      ...
      providers: [
        {
          ...
          useValue: new TilesService(storage, {
    ...
    try {
      await request(app.getHttpServer())
        .get("/tiles/soundg/v1/14/4544/6981.pbf")
        .expect(404);
      await request(app.getHttpServer())
        .get("/tiles/soundg/missing/14/0/0.pbf")
        .expect(404);
      await request(app.getHttpServer())
        .get("/tiles/soundg/v1/14/999999/0.pbf")
        .expect(404);
    } finally {
      await app.close();
    }
    ...
    assert.deepEqual(
      await readFile(path.join(versionPath, "tiles.pmtiles")),
      original,
    );
    assert.deepEqual(await readdir(path.join(directory, "soundg/versions")), [
      "v1",
    ]);
  } finally {
...
test("streaming generation handles filtered GeoJSONSeq larger than the GDAL stdin seek limit", async () => {
  const directory = await mkdtemp(
    path.join(tmpdir(), "maris-pmtiles-large-stream-"),
  );
  try {
      ...
      JSON.stringify({
        ...
        features: Array.from({ length: 10_000 }, (_, index) => ({
          ...
          geometry: {
            ...
            coordinates: [
              -80 + (index % 100) * 0.0001,
              25 + Math.floor(index / 100) * 0.0001,
            ],
          },
        })),
      }),
...
test("composed MVT contains only the cell selected by the metadata API at each point", async () => {
  const directory = await mkdtemp(
    path.join(tmpdir(), "maris-chart-composition-"),
  );
  try {
    const polygon = (w: number, e: number) => ({
      type: "Polygon",
      coordinates: [
        [
          [w, 25],
    ...
    const cells: CoverageCell[] = [
      {
        ...
        compilationScale: 80000,
        coverages: [{ category: 1, geometry: polygon(-81, -79) }],
      },
      {
        name: "harbor",
        edition: "1",
        ...
        coverages: [{ category: 1, geometry: polygon(-80.16, -80.14) }],
      },
    ];
    ...
    const features = [
      ...
      type: "Feature" as const,
      properties: { DEPTH: 21, SOURCE_CELL: name },
      geometry: { type: "Point" as const, coordinates: [Number(lon), 25.7] },
    }));
    await writeFile(
    ...
    const manifest = JSON.parse(
      await readFile(
        path.join(directory, "soundg/versions/composed/manifest.json"),
        "utf8",
      ),
    );
    assert.equal(manifest.selectionPolicy, CHART_SELECTION_POLICY);
    assert.equal(manifest.sourceFeatureCount, 2);
    const expectedFeatures = features.filter(
      (f) =>
        selection.at(f.geometry.coordinates as [number, number])?.name ===
        f.properties.SOURCE_CELL,
    );
    const expected = geojsonvt(
      { type: "FeatureCollection", features: expectedFeatures },
      { buffer: 64, extent: 4096, indexMaxZoom: 12, maxZoom: 16, tolerance: 3 },
    );
    const storage = new LocalChartStorageService(
      new ConfigService({ CHART_STORAGE_DIR: directory }),
    );
    await assertEquivalentMvt(
      ...
      Buffer.from(
        vtpbf.fromGeojsonVt({ soundings: expected.getTile(14, 4544, 6981)! }),
      ),
      directory,
      14,
      4544,
