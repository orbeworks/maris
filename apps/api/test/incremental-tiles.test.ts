...
test("catalog tile composition keeps the most detailed cell across immutable shards", async () => {
  ...
  const storage = {
    async getTile(_dataset: string, version: string) {
      return tiles.get(version);
    },
  ...
  const cells = [
    {
      ...
      edition: "1",
      updateNumber: 0,
      compilationScale: 100_000,
      issueDate: null,
      updateApplicationDate: null,
      coverages: [{ category: 1, geometry: coverage }],
    },
    {
      name: "DETAIL",
      edition: "1",
      updateNumber: 0,
      compilationScale: 10_000,
      issueDate: null,
      updateApplicationDate: null,
      coverages: [{ category: 1, geometry: coverage }],
    },
