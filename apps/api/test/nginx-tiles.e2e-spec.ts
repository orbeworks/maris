...
test(
  "Nginx rejects every legacy versioned tile route",
  {
  ...
  async () => {
    ...
    try {
      const template = await readFile(
        new URL("../../../ops/nginx/default.conf.template", import.meta.url),
        "utf8",
      );
      await writeFile(
      ...
      const address = execFileSync(docker, ["port", container, "8080"], {
        encoding: "utf8",
      }).trim();
      const get = (suffix: string) =>
        fetch(`http://${address}/tiles/soundg/${suffix}`);
      let ready = false;
      for (let i = 0; i < 30 && !ready; i++) {
        try {
          await get("v1/14/4543/6976.pbf");
          ready = true;
        } catch {
