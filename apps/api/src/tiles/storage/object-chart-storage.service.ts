...
export class ObjectChartStorageService implements ChartStorage {
  ...
  async getManifest(
    ...
    if (!loading) {
      ...
      if (this.manifestCache.size > 64) {
        const oldest = this.manifestCache.keys().next().value as
          | string
          | undefined;
    ...
    try {
      ...
      if (this.manifestCache.get(key) === loading)
  ...
  private async loadManifest(key: string, dataset: string, version: string) {
    const manifest = JSON.parse(
      await this.objects.getText(key),
    ) as TilesetManifest;
  ...
  async getTile(
    ...
    if (manifest.storageFormat !== "pmtiles") {
      throw new InternalServerErrorException(
        "S3 chart storage requires PMTiles artifacts",
      );
    }
    ...
    const reader = new PMTiles(
      {
        ...
        getBytes: (offset, length, signal) =>
          this.objects.getRange(archiveKey, offset, length, signal),
      },
      this.archiveCache,
    );
