...
const { VectorTile } = require(
  require.resolve("@mapbox/vector-tile", {
    paths: [vtpbfEntry],
  }),
) as {
  VectorTile: new (pbf: unknown) => { layers: Record<string, DecodedLayer> };
};
...
export class TilesService {
  ...
  private async getCatalogTile(
    ...
    const zoom = Number(z),
      column = Number(x),
      row = Number(y);
    if (zoom > 16 || column >= 2 ** zoom || row >= 2 ** zoom)
      throw new BadRequestException("Invalid tile coordinates");
  ...
  private async selectionForRevision(revision: number) {
    ...
    const loading = this.catalog
      .getPublishedCells("soundg", revision)
      ...
      for (let index = 0; index < layer.length; index += 1) {
        ...
        if (
          ...
          selected.updateNumber !==
            Number(feature.properties.SOURCE_UPDATE ?? 0)
        )
          continue;
        features.push({
