...
export class ObjectStorageService {
  ...
  constructor(config: ConfigService) {
    ...
    if (this.enabled && (!endpoint || !this.bucket || !accessKeyId || !secretAccessKey)) {
      throw new Error('CHART_STORAGE_BACKEND=s3 requires ENC_S3_ENDPOINT, ENC_S3_BUCKET, ENC_S3_ACCESS_KEY_ID and ENC_S3_SECRET_ACCESS_KEY');
    }
  ...
  private requireClient() {
    if (!this.client || !this.bucket) throw new Error('S3 object storage is not configured');
    return this.client;
  }

  async head(key: string) {
    return this.requireClient().send(new HeadObjectCommand({ Bucket: this.bucket, Key: key }));
  }
  ...
  async tryHead(key: string) {
    try {
      ...
      if (details.name === 'NotFound' || details.name === 'NoSuchKey' || details.$metadata?.httpStatusCode === 404) return null;
      throw error;
    }
  }
  ...
  async getRange(key: string, offset: number, length: number, signal?: AbortSignal) {
